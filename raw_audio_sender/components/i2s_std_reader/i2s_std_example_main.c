...
{
    int32_t *r_buf = (int32_t *)calloc(2*NUM_SAMPLES, sizeof(int32_t)); //stereo
    ...
    while (1) {
        ...
        if (isQueueReady && i2s_channel_read(rx_chan, r_buf, AUDIO_BUFF_SIZE, &r_bytes, 1000) == ESP_OK) {
            ...
            raw_audio_process(r_buf, p_buf, NUM_SAMPLES); // Process the audio data (assuming 32-bit samples)
